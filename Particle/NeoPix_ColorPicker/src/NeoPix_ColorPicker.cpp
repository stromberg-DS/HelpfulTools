...
int blueVal = 0;
int rgbVal = 0;
bool isUsingOnePot = false;   //choose if using one pot and button or 3 pots.

int baseColor = 0x991100;     //base color to test against.
...
void loop() {
  ...
  for(int j=PIXEL_COUNT/2; j<PIXEL_COUNT; j++){
    pixel.setPixelColor(j, baseColor);
  }
  pixel.show();
